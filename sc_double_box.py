...
class HeadOrientationAnalysis:
    ...
    def plot_data(self):
        ...
        axs[0].bar(self.bins[:-1], self.hist1, width=(self.bins[1] - self.bins[0]), bottom=0.0, color='green')
        ...
        axs[2].bar(self.bins[:-1], self.hist3, width=(self.bins[1] - self.bins[0]), bottom=0.0, color='red')
...
class TailMotionAnalysis:
    ...
    def process_data(self):
        ...
        self.angles3.extend([np.nan] * (max_len - len(self.angles3)))
        self.seconds.extend(range(len(self.angles1), max_len))
        self.colors.extend([' red'] * (max_len - len(self.colors)))
    ...
    def plot_tail_motion_smooth(self):
        ...
        handles = [Line2D([0], [0], color=c, marker='o', markersize=7, linestyle='', linewidth=6) for c in ['green', 'black', 'red']]
    ...
    def plot_chamber_durations(self):
        ...
        self.csv_file= os.path.join(self.output_folder, self.csv_file)
