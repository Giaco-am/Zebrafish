...
class HeadOrientationAnalysis:
    ...
    def analyze(self):
        ...
        axs[0].bar(bins[:-1], hist1, width=(bins[1] - bins[0]), bottom=0.0, color='red')
        axs[0].set_yticklabels([])
        axs[0].set_title('Zebrafish in SC')

        axs[1].bar(bins[:-1], hist2, width=(bins[1] - bins[0]), bottom=0.0, color='black')
...
class TailMotionAnalysis:
    ...
    def plot_tail_motion(self):
        ...
        plt.scatter(self.seconds, np.abs(self.angles_diff), c=self.colors, s=7)
