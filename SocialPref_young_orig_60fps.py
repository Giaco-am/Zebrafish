...
coordinates_path = r'young_coord_orig_60fps'
results_path = r'Young_orig_60fps\SocialPref'

for index,csv_file in enumerate(os.listdir(coordinates_path)):
    csv_file_path = os.path.join(coordinates_path, csv_file)

    subfolder_name = ' '.join(csv_file.split(' ')[1:3])
    ...
    # SC BOX
    x_box1, y_box1 = 550, 820
    x_box2, y_box2 = 775, 820
    x_box3, y_box3 = 775, 1140
    x_box4, y_box4 = 550, 1140
    ...
    # NSC BOX
    x_box5, y_box5 = 550, 110
    x_box6, y_box6 = 775, 110
    x_box7, y_box7 = 775, 435
    x_box8, y_box8 = 550, 435
    ...
    class HeadOrientationAnalysis:
        ...
        def plot_and_save_data(self):
            ...
            self.output_folder = 'Young_orig_60fps'
            ...
            for i, (p3, p4, p5, p6) in enumerate(zip(self.point3, self.point4, self.point5, self.point6)):
                ...
                if self.prev_angle is not None:
                    ...
                    if angle_diff <= 5:
                        ...
                        if self.stable_sequence_start is not None:
                            if (i - self.stable_sequence_start) >= 3 * 60:
                ...
                if self.prev_chamber is not None:
                    if self.prev_chamber != current_chamber:
                        self.chamber_changes.append(i/60)
                        self.chamber_durations.append((i - self.prev_time)/60)
                        self.prev_time = i
                    elif i == len(self.point3) - 1:
                        self.chamber_durations.append((i - self.prev_time + 1)/60)
                else:
                    self.prev_time = i
            ...
            for i, point in enumerate(self.point4):
                ...
                if new_box != current_box:
                    if current_box is not None:
                        raw_segments[current_box].append((segment_start_time, i / 60))
                        if (i / 60) - segment_start_time < 8 and previous_box is not None:
                            segments[previous_box][-1] = (segments[previous_box][-1][0], i / 60)
                        else:
                            segments[current_box].append((segment_start_time, i / 60))
                            previous_box = current_box
                        segment_start_time = i / 60
                    current_box = new_box
